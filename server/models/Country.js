...
class Country {
    constructor({country_id, name, capital, population, language, fun_fact, map_image_url}) { // This constructor will be used in the Controllers
    ...
    // Async because server is in Frankfurt - anything could happen
    static async getAll() {
    ...
    async update(data) {
        const response = await db.query("UPDATE country SET capital = $1 WHERE name = $2 RETURNING name, capital;", [data.capital, this.name])
        if (response.rows.length === 0) {
    ...
    async destroy() {
        let response = await db.query("DELETE FROM country WHERE name = $1 RETUNRING *;", [this.name])
        return new Country(response.rows[0])
    }
